...
def search_hyperparam(trial: optuna.trial.Trial) -> Dict[str, Any]:
    ...
    epochs = trial.suggest_int("epochs", low=25, high=25, step=25)
...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, int, float]:
    ...
    wandb_config.update(module_info)

    wandb.init(project='optuna-search', config=wandb_config, reinit=True)
    ...
    wandb.log({"f1_score": f1_score, "param_nums": params_nums, "mean_time": mean_time})
